...
blk_start <- 1 # in concentration unit
blk_end <- 1
blk_nb <- 200
...
unknown_smp_nb <- 500
integration_time <- 0.1 # seconds, a faire
replicate_nb <- 200
use_SE <- T
...
skip_drift <- F
skip_calib <- F
calib_non_affect_by_drift <- T
drift_non_affect_by_blk <- T
calib_non_affect_by_blk <- T
...
matrix_linear_regression <- function(x, y) {
  ...
  SSE <- t(mlr_residuals)%*%mlr_residuals
  df_regression <- (length(x) - 2)
  MSE <- SSE / df_regression
  cov_matrix <- as.numeric(MSE) * solve(t(X)%*%X)
...
for (iter in seq(iter_number)){
  ...
  sample_measured_var <- numeric()
    ...
    for (smp_sig in sample_theorical) {
    replicate_measured_values <- smp_sig + 
      rnorm(n = replicate_nb, mean = 0, sd = signal_sd * sensitivity) #counts
  ...
  if(calib_non_affect_by_drift == T) {
    ...
    var_beta_drift_ratio[index_calib] <- 0
  } else {}
  ...
  if(skip_calib == T) {
    ...
    calib_mlr_param <- matrix_linear_regression(x = standard_concentrations,
  ...
  samples_in_interval <- c(samples_in_interval, sum(test_concentration_in_interval))
  
  svMisc::progress(iter/iter_number*100)
  
  test_sd <- c(test_sd, sqrt(sample_blk_var[index_sample][10]))
}

paste(sum(index_blk), " blanks, ", sum(index_drift), " drift, ", sum(index_calib), " standards, ",
      replicate_nb, " replicates.")

print(mean(samples_in_interval)/sum(index_sample)*100)
```
