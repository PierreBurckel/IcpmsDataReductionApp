...
driftProcessing_server <- function(id, fileUpload, reactiveExpressions, indexCreation) {
  moduleServer(
    id = id,
    module = function(input, output, session) {
      ...
      observeEvent(input$setAsDriftIndex, {
        ...
        if (is.null(driftCorrectedElements())) {
          driftCorrectedElements(rep(FALSE, parameters()$analyteNumber))
        }
      ...
      output$driftPlot <- renderPlot({
        ...
        if (driftCorrectedElements()[input$driftTab_numericInput_analyteNumber] == TRUE && class(elementSpecificDriftModels()[[elementFullName]]) == "lm"){
          time_0 = driftTime[1]
      ...
      observeEvent(customIndex(), {
        ...
        updateSelectInput(session,"selectDriftIndex", label  = "Define drift index:", choices=names(customIndex()),names(customIndex())[1])
      })
      
      listOfElementSpecificDriftIndex <- reactive({
      ...
      elementSpecificDriftModels <- reactive({
      ...
      return(list(elementSpecificDriftModels = elementSpecificDriftModels,
                  ...
                  driftCorrectedElements = driftCorrectedElements))
    }
  )
